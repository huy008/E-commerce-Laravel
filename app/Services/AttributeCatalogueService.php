...
class AttributeCatalogueService extends BaseService implements AttributeCatalogueServiceInterface
{
     ...
     public function paginate($request)
     {
          ...
          $attributeCatalogue =
               $this->attributeCatalogueRepository->pagination(
                    [
     ...
     public function create($request)
     {
          ...
          try {
               ...
               if ($attributeCatalogue->id > 0) {
                    ...
                    $language = $this->attributeCatalogueRepository->createTranslatePivot($attributeCatalogue, $payloadLanguage);
     ...
     public function update($id, $request)
     {
          ...
          try {
               ...
               if ($flag == true) {
                    $payloadLanguage = $request->only(['name', 'description', 'content', 'meta_title', 'meta_keyword', 'meta_description', 'canonical']);
                    $payloadLanguage['language_id'] = $this->currentLanguage();
                    $payloadLanguage['attribute_catalogue_id'] = $id;
                    $attributeCatalogue->languages()->detach($payloadLanguage['language_id'], $id);
                    $response = $this->attributeCatalogueRepository->createTranslatePivot($attributeCatalogue, $payloadLanguage);
     ...
     public function destroy($id)
     {
          DB::beginTransaction();
          try {
               $attributeCatalogue = $this->attributeCatalogueRepository->destroy($id);
               $this->nestedset->Get('level ASC', 'order ASC');
               $this->nestedset->Recursive(0, $this->nestedset->Set());
               $this->nestedset->Action();
               DB::commit();
     ...
     public function updateStatus($post = [])
     {
          ...
          try {
               ...
               $Post = $this->attributeCatalogueRepository->update($post['modelId'], $payload);
     ...
     public function updateStatusAll($post = [])
     {
          ...
          try {
               ...
               $Post = $this->attributeCatalogueRepository->updateByWhereIn('id', $post['id'], $payload);
